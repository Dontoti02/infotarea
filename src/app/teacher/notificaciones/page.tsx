...
function NotificationsContent() {
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      const [notices, submissions] = await Promise.all([
        supabase
          .from("notices")
        ...
        supabase
          .from("submissions")
          .select("id, created_at, task_id, status")
          .order("created_at", { ascending: false })
          .limit(10),
      ]);
      ...
      (submissions.data ?? []).forEach((s) => {
        all.push({
          ...
          description: `Entrega #${s.id.slice(0, 8)} - ${s.status === "pending" ? "Pendiente de revisión" : "Revisada"}`,
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <h4 className="text-label-md font-bold text-on-surface truncate">{notif.title}</h4>
                  {!notif.read && (
                    <span className="w-2 h-2 rounded-full bg-primary flex-shrink-0" />
                  )}
                </div>
                <p className="text-body-md font-medium text-on-surface-variant mt-0.5">{notif.description}</p>
